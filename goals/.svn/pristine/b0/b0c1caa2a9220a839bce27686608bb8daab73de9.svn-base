...
public class GameresourceDAO extends BaseHibernateDAO {
	...
	public List<Gameresource> findOfProject(Integer projectId) {
		log.debug("finding all Gameresource instances");
		try {
			String queryString = "from Gameresource model where model.project.id = :pid";
			Query queryObject = getSession().createQuery(queryString);
	...
	public boolean checkName(Integer conceptId, Integer projectId, String name)
	{
		String queryString = "from Gameresource model where model.project.id = :pid  and lower(model.name) = :NAME";
		Query queryObject = getSession().createQuery(queryString);
		queryObject.setParameter("pid", projectId);
		queryObject.setParameter("NAME", name.toLowerCase());
		
		List<Gameresource> list = queryObject.list();
		if(list.size() == 0)
			return true;
		
		if(list.get(0).getId().equals(conceptId))
			return true;
