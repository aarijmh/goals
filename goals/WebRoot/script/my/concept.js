			...
			.on(
					...
					function(e) {
						...
						conceptPropertiesGrid.tableLoaded = function() {
							...
							if(!checkExists('#conceptPropertiesContent'))
								return;
...
function saveConcept() {
	...
	var saveText = $("#conceptBtnSave").text();
	if (!$("#concept").valid())
		return false;
	var nameData = {
	...
	$
			.ajax({
       		...
       		beforeSend: function(){
          			...
          			$("#conceptBtnSave").text('Please wait');
          			$("#conceptBtnSave").prop("disabled",true);
          			$("#conceptBtnClose").prop("disabled",true);
        	}
			})
			.done(
					function(data) {
	          			$("#conceptBtnSave").text(saveText);
	          			$("#conceptBtnSave").prop("disabled",false);
	          			$("#conceptBtnClose").prop("disabled",false);
	          			
						var obj = JSON.parse(data);
						if (!obj.messages) {
							alert("Concept with the same name already exists");
							return false;
						}
						...
						try {
							...
							for ( var i = 0; i < editableGrid.getRowCount(); i++) {
								...
								if (editableGrid.getRowValues(i).relation == 0
									...
									alert("You have to select a relation for concept relation");
									return false;
								}

								if (editableGrid.getRowValues(i).functions == 0
										|| editableGrid.getRowValues(i).functions == "") {
									alert("You have to select a function for concept relation");
									return false;
								}
