...
function saveLearner() {
	if (!$("#learner").valid())
		return false;
	try {
		var saveText = $("#learnerBtnSave").text();
		var superValues = [];
		...
		$.ajax({
			...
			beforeSend: function(){
      			...
      			$("#learnerBtnSave").text('Please wait');
      			$("#learnerBtnSave").prop("disabled",true);
      			$("#learnerBtnClose").prop("disabled",true);
    	}
		}).done(function(data) {
			$("#learnerBtnSave").text(saveText);
  			$("#learnerBtnSave").prop("disabled",false);
  			$("#learnerBtnClose").prop("disabled",false);
